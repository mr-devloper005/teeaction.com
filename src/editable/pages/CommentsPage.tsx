  ...
  return (
    <EditableSiteShell>
      <main className="mx-auto max-w-[1280px] px-4 py-10 sm:px-6 lg:px-8">
        <section className="rounded-[2rem] border border-[var(--slot4-line)] bg-white p-6 shadow-[0_18px_48px_rgba(18,27,52,0.08)] sm:p-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="inline-flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.3em] text-[var(--slot4-accent-fill)]">
            ...
            <button type="button" className="rounded-full border border-[var(--slot4-line)] px-4 py-2 text-sm font-black" onClick={refreshComments}>Refresh comments</button>
          </div>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative w-full sm:max-w-md">
              ...
              <input
                ...
                className="h-12 w-full rounded-[1rem] border border-[var(--slot4-line)] bg-[var(--slot4-surface-bg)] pl-9 pr-3 text-sm font-bold outline-none"
              />
            </div>
        ...
        {visibleComments.length ? (
          <section className="mt-8 grid gap-4">
            {visibleComments.map((item) => (
              <article key={`${item.articleSlug}-${item.id}`} className="rounded-[1.6rem] border border-[var(--slot4-line)] bg-white p-5 shadow-[0_14px_38px_rgba(18,27,52,0.06)]">
        ...
        {filtered.length > COMMENTS_PER_PAGE ? (
          <div className="mt-8 flex flex-wrap items-center justify-between gap-3 rounded-[1.6rem] border border-[var(--slot4-line)] bg-white p-4 text-sm text-[var(--slot4-muted-text)]">
